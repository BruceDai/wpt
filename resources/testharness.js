...
(function (global_scope)
{
    ...
    /**
     * Assert that each array property in `actual` is a number close enough within acceptable ULP distance
    ...
    function assert_array_approx_equals_nulp(actual, expected, nulp=1, dataType="float32")
    {
        ...
        assert_true(actual.length === expected.length,
                    `assert_array_approx_equals_nulp actual length ${actual.length} should be equal to expected length ${expected.length}`);
        ...
        for (let i = 0; i < actual.length; i++) {
            ...
            assert_true(distance <= nulp,
                        `The distance of ${distance} between ${actual[i]} and ${expected[i]} should be less than or equal to ${nulp}`);
        }
    }
