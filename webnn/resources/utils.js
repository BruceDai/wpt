...
const readFromNpy = async (fileName, precisionType) => {
  ...
  if (precisionType === 'float16') {
    ...
    return {buffer: u16Array, type: type.replace('float64', 'float16'), dimensions};
  } else if (precisionType === 'float32'){
    return {buffer: new Float32Array(typedArray), type: type.replace('float64', 'float32'), dimensions};
  } else {
